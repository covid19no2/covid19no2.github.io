var no2_2019;
var no2_2020;
proj4.defs("EPSG:9221", "+proj=aea +lat_0=-30 +lon_0=25 +lat_1=-22 +lat_2=-38 +x_0=1400000 +y_0=1300000 +ellps=WGS84 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs");
ol.proj.proj4.register(proj4);
var projection = new ol.proj.Projection({
	code: "EPSG:9221",
	extent: ol.proj.transformExtent([13, -38, 36, -20], "EPSG:4326", "EPSG:9221")
});
var map = new ol.Map({
	...
	view: new ol.View({
		...
		center: ol.proj.fromLonLat([25, -30], "EPSG:9221"),
		zoom: 1,
		maxZoom: 12,
		minZoom: 1,
		extent: ol.proj.transformExtent([13, -38, 36, -20], "EPSG:4326", "EPSG:9221")
...
fetch('images/no2/SouthAfricaNO2_2020.tif').then(response => response.arrayBuffer()).then(buffer => {
	...
	image.readRasters({
		...
		no2_2020 = new ol.layer.Image({
			source: new ol.source.ImageStatic({
				...
				imageExtent: [516276.7219, 725609.7546, 2226972.9842, 2180583.3797]
			}),
			extent: [516276.7219, 725609.7546, 2226972.9842, 2180583.3797],
			opacity: 0.5
		});
		map.addLayer(no2_2020);
	});

	fetch('images/no2/SouthAfricaNO2_2019.tif').then(response1 => response1.arrayBuffer()).then(buffer1 => {
