...
var projection = new ol.proj.Projection({
	code: "EPSG:2163",
	extent: ol.proj.transformExtent([-128, 20, -62, 53], "EPSG:4326", "EPSG:2163")
});
var map = new ol.Map({
	...
	view: new ol.View({
		...
		center: ol.proj.fromLonLat([-97, 35], "EPSG:2163"),
		zoom: 1,
		maxZoom: 12,
		minZoom: 1,
		extent: ol.proj.transformExtent([-128, 20, -62, 53], "EPSG:4326", "EPSG:2163")
...
fetch('images/no2/USANO2_2020.tif').then(response => response.arrayBuffer()).then(buffer => {
	...
	image.readRasters({
		...
		no2_2020 = new ol.layer.Image({
			source: new ol.source.ImageStatic({
				...
				imageExtent: [-2519772.3362, -2288983.5955, 3320824.7779, 984893.7106]
			}),
			extent: [-2519772.3362, -2288983.5955, 3320824.7779, 984893.7106],
			opacity: 0.5
		});
		map.addLayer(no2_2020);
	});

	fetch('images/no2/USANO2_2019.tif').then(response1 => response1.arrayBuffer()).then(buffer1 => {
